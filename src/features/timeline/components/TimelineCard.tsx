...
export default function TimelineCard({
  ...
  const needsReadMore =
    enableReadMore &&
    item.events.content.length > READ_MORE_LIMIT &&
    item.events.content.length - READ_MORE_REST >= READ_MORE_LIMIT
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    //only autoscroll on user clicks not programmatic .click() :)
  ...
  const cardId = `timeline-card-${item.id}`
  ...
  return (
    <Element name={cardId}>
      <div
        ...
        className={`rounded-card shadow-surface cursor-pointer transition-all duration-200
          ${s.panel}
          ${hoverHighlight ? 'hover:shadow-surfaceHover hover:-translate-y-0.5' : ''}
          ${active ? 'ring-active dark:ring-active-dark ring-2' : ''}
          bg-background dark:bg-background-dark text-context dark:text-context-dark`}
      >
        <div className="mb-2 flex items-start justify-between gap-2">
          <p className={`text-date dark:text-date-dark whitespace-nowrap flex-shrink-0 ${s.date}`}>
            {date}
          </p>
